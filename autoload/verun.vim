...
function verun#Compile(run, make) 
  ...
  " execute the program
  if a:run
    if s:TreatLocalVar('VEAutoClear', g:VEAutoClear)
      let l:clear = "clear; "
    else
      let l:clear = ""
    endif
    exec l:silent . "!" . l:clear . l:exec
    if has("gui_running") && !s:TreatLocalVar('VEExternalTerm', g:VEExternalTerm) " wait for keypress after execution inside gvim
      call getchar()
    endif
  endif
